...
# Разрешаем имя в путь ровно один раз, здесь.
#
# Тесты подменяют jq обёрткой вида `jq() { "$JQ" "$@"; }` - модули зовут его
# по имени, как и на роутере. Но если JQ это просто "jq", обёртка вызывает
# саму себя: рекурсия без дна. Локально не всплывало, потому что путь всегда
# передавался явно, а на CI переменная не задана - и dash обрывал файл на
# тысячной итерации, роняя весь набор.
#
# command -v здесь ещё находит бинарник, а не функцию: подмена появляется
# позже, в самом файле теста.
case "$JQ" in
*/*) ;;
*)
	_jq_resolved=$(command -v "$JQ" 2>/dev/null)
	[ -n "$_jq_resolved" ] && JQ="$_jq_resolved"
	;;
esac
